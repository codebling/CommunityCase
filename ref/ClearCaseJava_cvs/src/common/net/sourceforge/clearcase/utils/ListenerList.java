 ...
 * list that is optimized for frequent reads and infrequent writes. Copy on
 * write is used to ensure readers can access the list without synchronization
 * overhead. Readers are given access to the underlying array data structure for
 ...
 * Eclipse code base, except that it uses equality instead of identity to
 * compare listeners. This does not compromise efficiency (since listeners
 * addition/removal is not a frequent operation) and provides more flexibility
 * to clients.
 * </p>
 */
public class ListenerList {
	...
	/**
	 * Adds the given listener to this list. Has no effect if an equal listener
	 * is already registered.
	 * <p>
	 * This method is synchronized to protect against multiple threads adding or
	 * removing listeners concurrently. This does not block concurrent readers.
	 * 
	 * @param listener
	 *            the listener to add
	 */
		...
		for (int i = 0; i < oldSize; ++i) {
			if (listener.equals(listeners[i])) {
				if (oldSize == 1) {
					...
					System.arraycopy(listeners, i + 1, newListeners, i, oldSize
							- i - 1);
