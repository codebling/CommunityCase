 ...
 * be added to an interface using the <code>addDisposeListener</code> method
 * and removed using the <code>removeDisposeListener</code> method. When an
...
public interface DisposeListener extends EventListener {
	 ...
	 */
	public void interfaceDisposed(ClearCase impl);
}