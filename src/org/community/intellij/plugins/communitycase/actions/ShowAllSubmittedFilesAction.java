...
public class ShowAllSubmittedFilesAction extends AnAction implements DumbAware {
  ...
  public ShowAllSubmittedFilesAction() {
    super(Bundle.message("show.all.paths.affected.action.name"), null, IconLoader.getIcon("/icons/allRevisions.png"));
  }
  ...
  public void actionPerformed(AnActionEvent e) {
    ...
    if ((revision != null) && (revisionVirtualFile != null)) {
      final FileRevision Revision = ((FileRevision)revision);
      showSubmittedFiles(project, Revision, revisionVirtualFile);
    }
  }
  ...
  public static void showSubmittedFiles(final Project project, final String revision, final VirtualFile file) {
    Vcs.getInstance(project).runInBackground(new Task.Backgroundable(project, Bundle.message("changes.retrieving", revision)) {
      
      public void run(@NotNull ProgressIndicator indicator) {
        indicator.setIndeterminate(true);
        try {
          VirtualFile vcsRoot = Util.getRoot(file);
          final CommittedChangeList changeList = ChangeUtils.getRevisionChanges(project, vcsRoot, revision, false);
          if (changeList != null) {
            com.intellij.util.ui.UIUtil.invokeLaterIfNeeded(new Runnable() {
                public void run() {
                    AbstractVcsHelper.getInstance(project)
                            .showChangesListBrowser(changeList, ShowAllSubmittedFilesAction.getTitle(revision));
                }
            });
          }
        }
          ...
          com.intellij.util.ui.UIUtil.invokeLaterIfNeeded(new Runnable() {
              public void run() {
                  UiUtil.showOperationError(project, e, " show");
              }
          });
        }
      }
  ...
  private static String getTitle(final String revNumber) {
    return Bundle.message("paths.affected.title", revNumber);
  }
}
