...
public class RebaseEditor extends DialogWrapper {
   ...
   */
  protected RebaseEditor(final Project project, final VirtualFile Root, String file) throws IOException {
    super(project, true);
    myProject = project;
    myRoot = Root;
    setTitle(Bundle.getString("rebase.editor.title"));
    setOKButtonText(Bundle.getString("rebase.editor.button"));
    ...
    myViewButton.addActionListener(new ActionListener() {
      public void actionPerformed(final ActionEvent e) {
        ...
        RebaseEntry entry = myTableModel.myEntries.get(row);
        ShowAllSubmittedFilesAction.showSubmittedFiles(project, entry.getCommit(), Root);
      }
    });
  ...
  private void validateFields() {
    final List<RebaseEntry> entries = myTableModel.myEntries;
    if (entries.size() == 0) {
      setErrorText(Bundle.getString("rebase.editor.invalid.entryset"));
      setOKActionEnabled(false);
      return;
    }
    int i = 0;
    while (i < entries.size() && entries.get(i).getAction() == RebaseEntry.Action.skip) {
      i++;
    }
    if (i < entries.size() && entries.get(i).getAction() == RebaseEntry.Action.squash) {
      setErrorText(Bundle.getString("rebase.editor.invalid.squash"));
      setOKActionEnabled(false);
      return;
    }
    setErrorText(null);
  ...
  private class MyTableModel extends AbstractTableModel {
     ...
     * The entries
     */
    final List<RebaseEntry> myEntries = new ArrayList<RebaseEntry>();
    ...
    public String getColumnName(final int column) {
      switch (column) {
        case ACTION:
          return Bundle.getString("rebase.editor.action.column");
        case COMMIT:
          return Bundle.getString("rebase.editor.commit.column");
        case SUBJECT:
          return Bundle.getString("rebase.editor.comment.column");
        default:
    ...
    public Object getValueAt(final int rowIndex, final int columnIndex) {
      RebaseEntry e = myEntries.get(rowIndex);
      switch (columnIndex) {
        case ACTION:
          return new ListWithSelection<RebaseEntry.Action>(Arrays.asList(RebaseEntry.Action.values()), e.getAction());
        case COMMIT:
          return e.getCommit();
        case SUBJECT:
          return e.getSubject();
    ...
    private void setRowAction(Object aValue, int rowIndex, int columnIndex) {
      RebaseEntry e = myEntries.get(rowIndex);
      e.setAction((RebaseEntry.Action)aValue);
    ...
    public void load(final String file) throws IOException {
      String encoding = ConfigUtil.getLogEncoding(myProject, myRoot);
      ...
      while (s.hasMoreData()) {
        ...
        myEntries.add(new RebaseEntry(hash, comment));
      }
    }
    ...
    public void save(final String file) throws IOException {
      ...
      try {
        for (RebaseEntry e : myEntries) {
          if (e.getAction() != RebaseEntry.Action.skip) {
