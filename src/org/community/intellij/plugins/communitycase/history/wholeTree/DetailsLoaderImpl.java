...
import org.community.intellij.plugins.communitycase.history.browser.LowLevelAccess;
...
public class DetailsLoaderImpl implements DetailsLoader {
  ...
  public DetailsLoaderImpl(Project project) {
    myQueue = new BackgroundTaskQueue(project, " log details");
    myProject = project;
  ...
  private class Worker extends Task.Backgroundable {
    ...
    private Worker(@Nullable final Project project,
      ...
      super(project, "Load commits details", false, BackgroundFromStartOption.getInstance());
    ...
    public void run(@NotNull ProgressIndicator indicator) {
      ...
      if (! hashes.isEmpty()) {
        try {
          final Collection<org.community.intellij.plugins.communitycase.history.browser.Commit> result = myAccess.getCommitDetails(converted, myRefs.get(myAccess.getRoot()));
