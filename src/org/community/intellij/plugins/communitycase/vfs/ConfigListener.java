...
public interface ConfigListener extends EventListener {
  /**
   * This method is invoked when configuration changes for configured root.
   * If "./config" changes, only appropriate root is notified. If "~/.config" changes,
   * all configured roots are notified.
   * <p/>
   * When root configuration changes, the listeners are called for all new roots.
   *
   * @param Root    the affected root
   * @param configFile the changed configuration file, the parameter might be null if config file is missing
   */
  void configChanged(@NotNull VirtualFile Root, @Nullable VirtualFile configFile);
}
