...
public class FetchDialog extends DialogWrapper {
   ...
   */
  private RefspecPanel myRefspecs;
  /**
   * Fetch tags policy
   */
   ...
   */
  private static final String TAGS_POLICY_FOR_FETCHED_COMMITS = Bundle.getString("fetch.tags.policy.for.fetched.commits");
  /**
   * Fetch all tags policy
   */
  private static final String TAGS_POLICY_ALL = Bundle.getString("fetch.tags.policy.all");
  /**
   * Fetch no tags except explicitly listed
   */
  private static final String TAGS_POLICY_NONE = Bundle.getString("fetch.tags.policy.none");
   ...
   */
  public FetchDialog(final Project project, final List<VirtualFile> roots, final VirtualFile defaultRoot) {
    super(project, true);
    setTitle(Bundle.getString("fetch.title"));
    setOKButtonText(Bundle.getString("fetch.button"));
    UiUtil.setupRootChooser(project, roots, defaultRoot, myRoot, null);
    myProject = project;
    myRefspecs.setProject(project);
    myRefspecs.setReferenceSource(RefspecPanel.ReferenceSource.FETCH);
    setupRemotes();
  ...
  private void setupRemotes() {
    final ActionListener actionListener = new ActionListener() {
      public void actionPerformed(final ActionEvent e) {
        myRefspecs.setRoot(getRoot());
        updateRemotes();
      }
    };
    myRoot.addActionListener(actionListener);
  ...
  private void updateRemotes() {
    UiUtil.setupRemotes(myProject, getRoot(), myRemote, true);
  }

  /**
   * @return get current root
   */
   ...
   */
  public LineHandler fetchHandler() {
    LineHandler h = new LineHandler(myProject, getRoot(), Command.FETCH);
    h.addParameters("-v");
