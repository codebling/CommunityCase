...
public class BranchesSearcher {
  ...
  public BranchesSearcher(final Project project, final VirtualFile root, final boolean findRemote) throws VcsException {
    LOG.debug("constructing, root: " + root.getPath() + " findRemote = " + findRemote);
    final Set<Branch> usedBranches = new HashSet<Branch>();
    myLocal = Branch.current(project, root);
    LOG.debug("local: " + myLocal);
    if (myLocal == null) return;
    usedBranches.add(myLocal);

    Branch remote = myLocal;
    while (true) {
  ...
  public Branch getLocal() {
    return myLocal;
  }

  public Branch getRemote() {
    return myRemote;
  }
}
